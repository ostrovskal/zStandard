...
public:
    ...
    const zSize<T>& scale(float horz, float vert) { w *= vert; h *= vert; return *this; }
...
public:
	...
	zRect(T _x, T _y, T _w = 0, T _h = 0) : x(_x), y(_y), w(_w), h(_h) { }
	...
	T extent(bool vert) const { return (vert ? y + h : x + w); }
	...
	const zRect<T>& operator = (const zPoint<T>& p) { x = p.x; y = p.y; w = h = 0; return *this; }
	const zRect<T>& operator = (const zSize<T>& s) { w = s.w; h = s.h; x = y = 0; return *this; }
	const zRect<T>& operator * (T f) { x *= f; y *= f; w *= f; h *= f; return *this; }
	...
	const zRect<T>& scale(float horz, float vert) { w *= vert; h *= vert; return *this; }
    zRect<T> padding(const T& horz, const T& vert) { return zRect<T>(x + horz, y + vert, w - horz * 2, h - vert * 2); return *this; }
	zRect<T> padding(const zPoint<T> &p) { return zRect<T>(x + p.x, y + p.y, w - p.x * 2, h - p.y * 2); return *this; }
	zRect<T> padding(const zRect<T> &r) { return zRect<T>(x + r.x, y + r.y, w - (r.w + r.x), h - (r.h + r.y)); return *this; }
...
public:
	...
	auto operator + () const { return *this; }
	// бинарные
	auto operator - () const { return zMatrix(-_11, -_12, -_13, -_14, -_21, -_22, -_23, -_24, -_31, -_32, -_33, -_34, -_41, -_42, -_43, -_44); }
	auto operator + (float f) const { zMatrix(_v1 + f, _v2 + f, _v3 + f, _v4 + f); }
	auto operator - (float f) const { zMatrix(_v1 - f, _v2 - f, _v3 - f, _v4 - f); }
	auto operator * (float f) const { zMatrix( _v1 * f, _v2 * f, _v3 * f, _v4 * f); }
	auto operator / (float f) const { zMatrix( _v1 / f, _v2 / f, _v3 / f, _v4 / f); }
	auto operator + (const zMatrix& m) const { zMatrix(_v1 - m._v1, _v2 - m._v2, _v3 - m._v3, _v4 - m._v4); }
	auto operator - (const zMatrix& m) const { zMatrix(_v1 - m._v1, _v2 - m._v2, _v3 - m._v3, _v4 - m._v4); }
	auto operator / (const zMatrix& m) const { zMatrix(_v1 / m._v1, _v2 / m._v2, _v3 / m._v3, _v4 / m._v4); }
	zMatrix operator * (const zMatrix& m) const { return zMatrix(z_mtx_mtx(*this, m)); }
	auto operator += (float f) { _v1 += f; _v2 += f; _v3 += f; _v4 += f; return *this; }
	auto operator -= (float f) { _v1 -= f; _v2 -= f; _v3 -= f; _v4 -= f; return *this; }
	auto operator *= (float f) { _v1 *= f; _v2 *= f; _v3 *= f; _v4 *= f; return *this; }
	auto operator /= (float f) { _v1 /= f; _v2 /= f; _v3 /= f; _v4 /= f; return *this; }
	auto operator += (const zMatrix& m) { _v1 += m._v1; _v2 += m._v2; _v3 += m._v3; _v4 += m._v4; return *this; }
	auto operator -= (const zMatrix& m) { _v1 -= m._v1; _v2 -= m._v2; _v3 -= m._v3; _v4 -= m._v4; return *this; }
	...
	bool isIdent() const { return memcmp(&_identiry, *this, 16) == 0; }
	// специальные
	...
	// единичная
	const zMatrix& identity() { memcpy(*this, &_identiry, sizeof(zMatrix)); return *this; }
	// матрица позиции
	const zMatrix& translate(float x, float y, float z);
	auto translate(const zVec3& v) { return translate(v.x, v.y, v.z); }
	// матрица масштабирования
	const zMatrix& scale(float x, float y, float z);
